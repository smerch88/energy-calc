function calculateBoilerEnergyConsumption(waterVolume, initialTemp, targetTemp, efficiency, costPerKWh, hotWaterCostPerCubicMeter, coldWaterCostPerCubicMeter, subscriptionFee, nightRateFactor = 1) {
...
function calculateForSummer(waterVolume, targetTemp, efficiency, costPerKWh, hotWaterCostPerCubicMeter, coldWaterCostPerCubicMeter, subscriptionFee, nightRateFactor = 1) {
    const initialTemp = 15; // Початкова температура води влітку (°C)
    return calculateBoilerEnergyConsumption(waterVolume, initialTemp, targetTemp, efficiency, costPerKWh, hotWaterCostPerCubicMeter, coldWaterCostPerCubicMeter, subscriptionFee, nightRateFactor);
}

// Викликаємо для зимового періоду (температура холодної води = 5°C)
function calculateForWinter(waterVolume, targetTemp, efficiency, costPerKWh, hotWaterCostPerCubicMeter, coldWaterCostPerCubicMeter, subscriptionFee, nightRateFactor = 1) {
...
// Розрахунок для літа
const summerResult = calculateForSummer(waterVolume, targetTemp, efficiency, costPerKWh, hotWaterCostPerCubicMeter, coldWaterCostPerCubicMeter, subscriptionFee, nightRateFactor);
console.log(`Літнє енергоспоживання для нагрівання води: ${summerResult.energyConsumption.toFixed(2)} кВт*год`);
console.log(`Літня загальна вартість нагріву бойлером: ${summerResult.totalCostInUAH.toFixed(2)} грн`);
console.log(`Літня вартість гарячої води з мережі: ${summerResult.networkHotWaterCostInUAH.toFixed(2)} грн`);

// Розрахунок для зими
const winterResult = calculateForWinter(waterVolume, targetTemp, efficiency, costPerKWh, hotWaterCostPerCubicMeter, coldWaterCostPerCubicMeter, subscriptionFee, nightRateFactor);
console.log(`Зимове енергоспоживання для нагрівання води: ${winterResult.energyConsumption.toFixed(2)} кВт*год`);
console.log(`Зимова загальна вартість нагріву бойлером: ${winterResult.totalCostInUAH.toFixed(2)} грн`);
console.log(`Зимова вартість гарячої води з мережі: ${winterResult.networkHotWaterCostInUAH.toFixed(2)} грн`);
